...
namespace FractalPainting.App.Fractals
{
	public class DragonPainter
	{
		...
		public DragonPainter(IImageHolder imageHolder, DragonSettings dragonSettings
		, Palette palette)
		{
		...
		public void Paint()
		{
            ...
            using (var graphics = imageHolder.StartDrawing())
			{
				...
				var r = new Random();
				var cosa = (float) Math.Cos(dragonSettings.Angle1);
				var sina = (float) Math.Sin(dragonSettings.Angle1);
				var cosb = (float) Math.Cos(dragonSettings.Angle2);
				var sinb = (float) Math.Sin(dragonSettings.Angle2);
				var shiftX = dragonSettings.ShiftX*size*0.8f;
				var shiftY = dragonSettings.ShiftY*size*0.8f;
				var scale = dragonSettings.Scale;
				var p = new PointF(0, 0);
				foreach (var i in Enumerable.Range(0, dragonSettings.IterationsCount))
				{
